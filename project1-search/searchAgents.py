...
class CornersProblem(search.SearchProblem):
    ...
    def updateFoodStatus(self, state):
        posit = (state[0], state[1])
        return (
            0 if posit == (1, 1) else state[-1][0],
            0 if posit == (1, self.top) else state[-1][1],
            0 if posit == (self.right, 1) else state[-1][2],
            0 if posit == (self.right, self.top) else state[-1][3],
        )
                ...
                successors.append(
                    (
                        (
                            nextx,
                            nexty,
                            self.updateFoodStatus((nextx, nexty, state[-1])),
                        ),
                        action,
                        1,
                    )
...
def cornersHeuristic(state, problem):
    ...
    unvisited_corners = [corners[i] for i in range(4) if visited_corners[i] == 1]
...
def foodHeuristic(state, problem):
    ...
    disMeasure = util.manhattanDistance

    def findClosetFood(posit, fs):
        if len(fs) == 0:
            return None, 0

        PQ = util.PriorityQueue()
        assert PQ.isEmpty()
        mDistances = [disMeasure(food, posit) for food in fs]
        [PQ.push(food, mD) for food, mD in zip(fs, mDistances)]
        return PQ.pop(), min(mDistances)

    corners = [
        (1, 1),
        (1, problem.walls.height),
        (problem.walls.width, 1),
        (problem.walls.width, problem.walls.height),
    ]

    foodClosettoCorners = [findClosetFood(corner, foods) for corner in corners]
    pq = util.PriorityQueue()
    [
        pq.push(f, mazeDistance(position, f, problem.startingGameState))
        for f, d in foodClosettoCorners
    ]

    theFood = pq.pop()
    h0 = mazeDistance(position, theFood, problem.startingGameState)
