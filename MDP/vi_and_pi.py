...
def policy_evaluation(P, nS, nA, policy, gamma=0.9, tol=1e-3):
    ...
    prev_value_function = np.full(nS, np.inf)
    count = 0
    while np.max(np.abs(value_function - prev_value_function)) > tol:
        ...
        for s in range(nS):
            value_function[s] = sum(
                [
                    p * (r + gamma * prev_value_function[s_])
                    for p, s_, r, _ in P[s][policy[s]]
                ]
            )
    print(f"Policy evaluation converged in {count} iterations")
    ############################
...
def policy_improvement(P, nS, nA, value_from_policy, policy, gamma=0.9):
    ...
    for s in range(nS):
        new_policy[s] = np.argmax(
            [
                sum(
                    [p * (r + gamma * value_from_policy[s_]) for p, s_, r, _ in P[s][a]]
                )
                for a in range(nA)
            ]
        )
...
def policy_iteration(P, nS, nA, gamma=0.9, tol=10e-3):
    ...
    prev_policy = np.ones(nS, dtype=int)  # Arbitrary initialization
    while not np.all(policy == prev_policy):
...
def value_iteration(P, nS, nA, gamma=0.9, tol=1e-3):
    ...
    while np.max(np.abs(value_function - prev_value_function)) > tol:
        ...
        for s in range(nS):
            ...
            value_function[s] = max(candiate_values)

            policy[s] = np.argmax(candiate_values)
